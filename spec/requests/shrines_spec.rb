...
RSpec.describe 'Shrines', type: :request do
  ...
  describe 'POST /shrines' do
    context 'ログインしている場合' do
      ...
      it '神社が保存される' do
        shrine_params = attributes_for(:shrine, name: '登録した神社名')

        expect {
          post shrines_path, params: { shrine: shrine_params }
        }.to change(Shrine, :count).by(1)

        shrine = user.shrines.order(:created_at).last

        expect(response).to redirect_to(shrine_path(shrine))
        expect(shrine.name).to eq '登録した神社名'
      end
  ...
  describe 'GET /shrines/:id/edit' do
    context 'ログインしている場合' do
      before do
        sign_in user
      end
      ...
      it '200ステータスが返ってくる' do
        ...
        get edit_shrine_path(shrine)

        expect(response).to have_http_status(:ok)
      end

      it '他人の神社の編集画面にはアクセスできない' do
        other_user = create(:user)
        other_shrine = create(:shrine, user: other_user)

        get edit_shrine_path(other_shrine)
  ...
  describe 'PATCH /shrines/:id' do
    context 'ログインしている場合' do
      ...
      it '不正な値の場合、神社が更新されない' do
        shrine = create(:shrine, user: user)
        original_name = shrine.name
        ...
        expect(shrine.reload.name).to eq original_name
      end
      ...
      it '他人の神社は更新できない' do
        ...
        patch shrine_path(other_shrine), params: {
          shrine: { name: '不正に更新' }
        }

        expect(response).to have_http_status(:not_found)
        expect(other_shrine.reload.name).not_to eq '不正に更新'
      end
    end
    ...
    context 'ログインしていない場合' do
      it 'ログイン画面に遷移する' do
        ...
        expect(response).to redirect_to(new_user_session_path)
        expect(shrine.reload.name).not_to eq '更新後の神社名'
      end
    end
  ...
  describe 'DELETE /shrines/:id' do
    context 'ログインしている場合' do
      ...
      it '他人の神社は削除できない' do
        ...
        expect {
          delete shrine_path(other_shrine)
        }.not_to change(Shrine, :count)
